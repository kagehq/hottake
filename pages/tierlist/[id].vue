  ...
  <div class="min-h-screen bg-black text-white">
    <div class="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 class="text-2xl font-bold text-white mb-1">{{ tierListData?.state?.title || 'Trumptier' }}</h1>
...
const pageTitle = computed(() => tierListData.value?.state?.title || 'Trumptier');
const pageDescription = computed(() => `Check out this tier list: ${tierListData.value?.state?.title || 'Trumptier'}`);
